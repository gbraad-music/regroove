...
InputAction parse_action(const char *str) {
    ...
    if (strcmp(str, "fx_reverb_room_size") == 0) return ACTION_FX_REVERB_ROOM_SIZE;
    if (strcmp(str, "fx_reverb_damping") == 0) return ACTION_FX_REVERB_DAMPING;
    if (strcmp(str, "fx_reverb_mix") == 0) return ACTION_FX_REVERB_MIX;
    ...
    if (strcmp(str, "fx_reverb_toggle") == 0) return ACTION_FX_REVERB_TOGGLE;
...
const char* input_action_name(InputAction action) {
    switch (action) {
        ...
        case ACTION_FX_REVERB_ROOM_SIZE: return "fx_reverb_room_size";
        case ACTION_FX_REVERB_DAMPING: return "fx_reverb_damping";
        case ACTION_FX_REVERB_MIX: return "fx_reverb_mix";
        ...
        case ACTION_FX_REVERB_TOGGLE: return "fx_reverb_toggle";
...
void input_mappings_reset_defaults(InputMappings *mappings) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        snprintf(mappings->trigger_pads[8 + i].parameters, sizeof(mappings->trigger_pads[8 + i].parameters), "%d", i);
    }
...
int input_mappings_save(InputMappings *mappings, const char *filepath) {
    ...
    fprintf(f, "#   - file_load_byname: filename (e.g., song.it)\n");
    fprintf(f, "#   - channel_mute: channel_index (e.g., 0)\n");
    ...
    fprintf(f, "#   pad2 = file_load_byname,song.it,48,-1      # Loads song.it when MIDI note 48 received\n");
    fprintf(f, "#   pad3 = channel_mute,0,50,-1                # Mutes channel 0 when MIDI note 50 received\n");
    fprintf(f, "#   pad4 = play_pause,,52,-1                   # Play/pause when MIDI note 52 received\n\n");
