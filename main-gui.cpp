...
static UIMode ui_mode = UI_MODE_PADS;  // Start in rhythmic/performance mode
...
static UIMode last_ui_mode = UI_MODE_PADS;
...
static void format_pad_action_text(InputAction action, int parameter, char *line1, size_t line1_size,
    ...
    switch (action) {
        ...
        case ACTION_FX_REVERB_TOGGLE: snprintf(line1, line1_size, "REVERB\nTOGGLE"); break;
        ...
        if (!already_mapped && common_state->input_mappings->midi_count < common_state->input_mappings->midi_capacity) {
            ...
            if (learn_target_type == LEARN_ACTION &&
                (learn_target_action == ACTION_CHANNEL_VOLUME ||
                 ...
                 learn_target_action == ACTION_FX_REVERB_ROOM_SIZE ||
                 learn_target_action == ACTION_FX_REVERB_DAMPING ||
...
static void ShowMainUI() {
    ...
    const float BASE_SPACING = 24.0f * g_dpi_scale;  // Reduced from 26.0f for tighter fit
    ...
    if (ui_mode == UI_MODE_VOLUME) {
                ...
                for (int a = ACTION_NONE; a < ACTION_MAX; a++) {
                    ...
                    if (ImGui::Selectable(input_action_name(act), new_midi_action == act)) {
                        ...
                        if (act == ACTION_CHANNEL_VOLUME ||
                            ...
                            act == ACTION_FX_REVERB_ROOM_SIZE ||
                            act == ACTION_FX_REVERB_DAMPING ||
        ...
        if (!effects) {
            ...
            const float fx_spacing = 12.0f * g_dpi_scale;  // Reduced from 16.0f for tighter fit
            int col_index = 0;
